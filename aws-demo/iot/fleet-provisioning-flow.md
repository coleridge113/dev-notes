...
Embed Amazon Certificate Authority (rootCA) in app level

RS (login) -> Backend (get jwt) -> RS (w/ jwt) -> Backend (gets temporary certs from template) / Lambda -> RS (w/ claim) -> IOT Core (exchange claim for permanent cert & private key) -> RS creates MQTT client
